...
bool RHGenericDriver::waitPacketSent(uint16_t timeout)
{
    ...
    uint64_t starttime = time_us_64(); // Get current time in microseconds
    uint64_t timeout_us = timeout * 1000; // Convert milliseconds to microseconds

    while ((time_us_64() - starttime) < timeout_us)
    {
        if (_mode != RHModeTx) // Check if transmission has finished
            return true;

        tight_loop_contents(); // This is similar to YIELD in Arduino
    }
    return false;
}
